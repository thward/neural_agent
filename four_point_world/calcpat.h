...
void calcDesiredInputOutputPatterns(NODE_TYPE_P node_tail, int size_of_pattern, PATTERN_TYPE_P input_pattern_current_head,PATTERN_TYPE_P output_pattern_current_head)
{
  ...
  if (greatestNodeValue < patternThreshold)
  {
    fprintf(log_file_p,"LAZY !!! \n");
    ...
    )
    {
      ...
      input_pattern_current_head -> next -> next -> value  = 0 ;
      // + invert output
      calcOutputPattern(node_tail, size_of_pattern, output_pattern_current_head, 0.0,0,0,0,Test,patternThreshold); //wrong, but let's move !
    }
    ...
    {
      input_pattern_current_head -> value = 0;
      input_pattern_current_head -> next -> value  = 0 ;  //let's just move so we don't get stuck here !
      input_pattern_current_head -> next -> next -> value  = 0 ;
      // + reinforce output
